...
const index = () => {
    ...
    const filteredNav = filterNav(routerLinks);
    console.log(filteredNav);
    
    
    return (
        <header className="h-[80px]">
            <nav className="bg-white border-gray-200 px-4 lg:px-6 py-5 dark:bg-gray-800 h-[80px]">
                <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                    <a href="/" className="flex items-center">
                        <img src={NavLogo} className="mr-3 h-6 sm:h-9" alt="Flowbite Logo" />
                        <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">React Router</span>
                    </a>
                        ...
                        <ul className="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
                            {
                                ...
                                routerLinks.map((item) => {
                                    return (
                                        <li key={item.id}>
                                            <NavLink to={item.path} className="block py-2 pr-4 pl-3  rounded bg-primary-700 lg:bg-transparent lg:text-primary-700 lg:p-0 dark:text-gray-400" aria-current="page">
                                                {item.title}
                                            </NavLink>
                                        </li>
                                    )
